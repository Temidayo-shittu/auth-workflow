...
const register = async (req, res) => {
  ...
  if (emailAlreadyExists) {
    throw new CustomError.BadRequestError('Email already exists'); //08053672362
  }
...
const login = async (req, res) => {
  ...
  if(!user.isVerified) throw new CustomError.UnauthenticatedError('Please verify email');
...
const forgotPassword= async(req,res)=>{
  ...
  if(!email) throw new CustomError.UnauthenticatedError('Please provide valid email');
  const user= await User.findOne({email})
  if(user){
...
const resetPassword= async(req,res)=>{
  ...
  if(!token || !email || !password) throw new CustomError.BadRequestError('Please provide all values');
  const user= await User.findOne({email})
  if(user){
    const currrentDate= new Date
    if(user.passwordToken===createHash(token) && user.passwordTokenExpirationDate>currrentDate){
